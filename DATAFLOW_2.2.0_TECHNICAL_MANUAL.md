# DataFlow 2.2.0 — Technical Manual
...
> **Version:** 2.2.0 (current codebase as of April 2026)
...
### Architectural Direction in 2.2.0
...
The `services/` package is the main extraction area of version 2.2.0. Responsibility is now split into focused modules rather than concentrated inside `dataflow.py`.
...
**Notable 2.2.0 orchestration characteristics:**
...
The code scans sibling `.db` files and reads them conservatively in read-only mode. RFQ aggregation uses `ATTACH DATABASE` for SQL-side union logic; VSM and Derisking aggregation use direct read-only connections.
...
The footer currently renders the application version string `v.2.2.0`.
...
The current 2.2.0 codebase uses `tr(...)` extensively across:
...
`development/dev_tools/tools_build_WIN/msix/AppxManifest.xml` is present and currently declares an MSIX package identity version of `2.0.1.0`. This is the literal current file content and should be treated as packaging metadata, not as the application runtime version indicator.
...
The repository supports **manual and fixture-driven validation**, not a repository-contained automated regression suite for the current 2.2.0 application logic.
...
6. splash and first-run dialogs run if required.
...
The 2.2.0 codebase keeps backward-compatible orchestration in `dataflow.py` while extracting isolated concerns into services. The direction is modularization, not framework replacement.
